...
from app.classes.test_row_handler.AbstractTestRowHandler import AbstractTestRowHandler
...
from constants import RESPONSE_NO_ANSWER, DELTA_TIME
...
class StateTestRowHandler(AbstractTestRowHandler):
    ...
    def handle(self,
        ...
        if len(row) == 2 and (';' in row[1]):
            ...
            copy_row = row.copy()
            head, sep, tail = copy_row[1].rpartition('?')
            try:
                if sep:
                    copy_row[1] = head
                if tail:
                    try:
                        ...
                        error_message = f'ERROR! Couldn`t convert string [{tail}] in integer'
                        self.test_task.write_test_log_report(error_message)
                        raise BadSendMessageException(error_message)
                message = self.get_message_from_row(copy_row)
                if sep:
                    ...
                    while (datetime.datetime.now() < finish_time):
                        try:
                            ...
                            self.check_response(response, copy_row)
    ...
    def get_message_from_row(self, row: list[str]) -> str:
        ...
        if (alias_object in
            ...
            message = f'ERROR! Couldn`t find object [{alias_object}] in station model [{self.test_task.station.name}]'
    ...
    def check_response(self,
        ...
        for record in response_items[2:]:
            ...
            if len(items) == 2:
                ...
                message = f'ERROR! Got wrong amount of object`s colors [{len(items)}]'
                self.test_task.write_test_log_report(message)
                raise BadResponsedMessageException(message)
            ...
            if not permanent_color or not blink_color:
                message = f'ERROR! Couldn`t find color [{id_permanent_color}] or [{id_blink_color}] in station.colors'
                self.test_task.write_test_log_report(message)
                raise BadResponsedMessageException(message)
        ...
        if id_object not in self.test_task.station.states:
            message = f'ERROR! Couldn`t find object [{id_object}] in station.objects'
            self.test_task.write_test_log_report(message)
            raise BadResponsedMessageException(message)
        for lr in self.test_task.station.states[id_object][state]:
            if lr not in layers:
                message = f'ERROR! Couldn`t find layer [{lr}] in station.states for state [{state}]'
                self.test_task.write_test_log_report(message)
                raise BadResponsedMessageException(message)
            if layers[lr] != (self.test_task.station.states[id_object][state][lr]):
                message = f'ERROR! For object [{self.test_task.station.objects[id_object]}] in \
state [{state}] for layer [{lr}] expected state \
...
got [{layers[lr].blink_color}: {layers[lr].permanent_color}]'
    ...
    def get_finish_time(self, row: list[str]) -> str:
        pass
